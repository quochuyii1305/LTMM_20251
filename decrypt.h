...
// Lưu ý: RC4 đối xứng, nên giải mã = mã hoá (XOR hai lần = bản gốc)
void RC4_Decrypt(uint8_t *ciphertext, int ciphertext_len, 
    ...
    // Bước 2: Hoán vị khởi tạo (giống như mã hoá)
    RC4_KeyScheduling(&state, key, keylen);
    
    // Bước 3: Tạo dòng khóa và giải mã
